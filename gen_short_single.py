...
for mname, (model, is_sca, gids) in models.items():
    ...
    n = 90
    ax.plot(range(n), yt[sample_idx, :n], "b-", label="Ground Truth", lw=1.2)
    ax.plot(range(n), yp[sample_idx, :n], "r--", label="Prediction", lw=1.2)
    ax.set_xlabel("Days")
    ax.set_ylabel("Active Power (normalized)")
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    out = os.path.join(REPORT_FIG, f"pred_vs_gt_short_{mname}_single.pdf")
